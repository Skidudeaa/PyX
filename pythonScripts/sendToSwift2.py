...
# Function to adjust search term based on the API requirements
def adjust_search_term_for_api(api_name, search_term):
    if api_name == 'Apple Music':
        return search_term.replace(' ', '+')
    elif api_name == 'Genius':
        return search_term
    elif api_name == 'Musixmatch':
        return search_term.replace(' ', '%20')
    else:
        return search_term
...
def get_song_urls(search_term):
    search_api = 'https://api.music.apple.com/v1/catalog/us/search'
    search_params = {'term': search_term, 'types': 'songs'}
    ...
    song_urls = list(set(song['attributes']['url'] for song in songs_data))
    song_title = songs_data[0]['attributes']['name']
    artist_name = songs_data[0]['attributes']['artistName']
    return song_urls, song_title, artist_name


# Semantic Truncate Function
def semantic_truncate(text, max_length=900):
    model = Summarizer()
    summary = model(text)
...
def get_song_id(search_term, api_key):
    ...
    if response.status_code != 200:
        print(f"Failed to get song ID. HTTP Status Code: {response.status_code}")
        return None
    try:
        response_json = response.json()
        for hit in response_json["response"]["hits"]:
            if hit["result"]["primary_artist"]["name"].lower() in search_term.lower():
...
def get_song_details_and_annotations(search_term, api_key):
    ...
    for referent in referents:
        ...
        truncated_annotation = semantic_summary[:700]  
        song_data['annotations'].append({
            'referent': referent['fragment'],
            'annotation': truncated_annotation
        })
    return song_data, song_details['title'], song_details['primary_artist']['name']
...
class Musixmatch(LRCProvider):
    ...
    def get_lrc(self, search_term: str) -> Optional[str]:
        ...
        for track in tracks:
            ...
            if score > best_score and score > 50:  # Increase the threshold to 60
                best_score = score
                best_match = track
...
def fetch_lyrics_with_timestamps(user_token, search_term):
    ...
    lyrics = musixmatch_provider.get_lrc(search_term)  # Removed the extra parameter
    lyrics_data = None
    if lyrics:
...
def main():
    # Take user input for song and/or artist
    search_term = input("Enter song title and/or artist name: ")

    # Adjust the search term for each API
    apple_music_search_term = adjust_search_term_for_api('Apple Music', search_term)
    genius_search_term = adjust_search_term_for_api('Genius', search_term)
    musixmatch_search_term = adjust_search_term_for_api('Musixmatch', search_term)

    # Apple Music API: Fetch song URLs and resolved song title and artist name
    song_urls, resolved_song_title, resolved_artist_name = get_song_urls(apple_music_search_term)

    if song_urls is None:
        print("No songs found on Apple Music. Exiting.")
        return
    
    # For each song URL, attempt to download the video. If video is not found, download the album artwork.
    for url in song_urls:
        playlist_url = fetch_playlist_url(url)
        if playlist_url is None:
            download_album_artwork(url, search_term)
        else:
            ...
            if variant_playlist_url is None:
                ...
                if not segment_urls:
                    ...
                    download_video_segments(segment_urls, search_term)
...
# Genius API: Fetch song details and annotations
    try:
        song_data, genius_song_title, genius_artist_name = get_song_details_and_annotations(genius_search_term, GENIUS_API_KEY)
        if not all([song_data, genius_song_title, genius_artist_name]):
            print("Failed to get all song details and annotations.")
            return  # Exit the function if you cannot proceed without these details
    except Exception as e:
        print(f"An error occurred: {e}")
        return  # Exit the function if an error occurs


    # Fetch lyrics with Musixmatch
    lyrics_data = fetch_lyrics_with_timestamps(MUSIXMATCH_USER_TOKEN, musixmatch_search_term)

    if lyrics_data is None:
        print("No lyrics data found. Exiting.")
        return
