...
def download_image(image_url, image_path):
    response = response = requests.get(image_url, stream=True)
...
def search_song(song_title, developer_token):
    ...
    response = response = requests.get('https://api.music.apple.com/v1/catalog/us/search', headers=headers, params=params)
print('API Request URL:', response.url)
print('API Response Status Code:', response.status_code)
print('API Response Content:', response.content.decode('utf-8'))
response
